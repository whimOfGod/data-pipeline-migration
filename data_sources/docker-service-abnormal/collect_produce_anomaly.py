...
bootstrap_servers = "163.173.170.108:9092" #TFX-10
...
def collect_publish(producer):
    ...
    number_of_samples = 5000 #To have approximately 40 minutes of process collecting and publishing  1 sample per 100 ms 
    sampling_rate = 100/1000
    start_anamolay_process = False
    start = True
    process = multiprocessing.Process(target=compute_regression_in_parallel_to_simulate_load)
    while start:
        try:
            ...
            publish_to_topic(producer, cpu_producer_topic, metrics_json)
            ...
            if sample_counting > number_of_samples * (3/4):
                
                if start_anamolay_process == False:
                    start_anamolay_process = True
                    process.start()
